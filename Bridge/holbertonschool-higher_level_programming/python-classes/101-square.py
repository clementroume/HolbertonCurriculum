...
class Square:
    ...
    def __str__(self):
        """
        Provides a string representation of the square, including position.

        Returns:
            str: A string that visually represents the square.
        """
        if self.size == 0:
            return ""
        ...
        lines.extend(["" for _ in range(vertical_space)])
        # Add rows of the square
        for _ in range(self.size):
            lines.append(" " * horizontal_space + "#" * self.size)
        return "\n".join(lines)
    ...
    def my_print(self):
        ...
        if self.size == 0:
            ...
            print(str(self))  # Leverage __str__ method for printing
